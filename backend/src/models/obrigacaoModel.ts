...
export class ObrigacaoModel {
  // Criar obrigação
  criar(obrigacao: Omit<Obrigacao, 'id' | 'criadoEm' | 'atualizadoEm'>): Obrigacao {
    const id = uuidv4();
    ...
    if (obrigacao.recorrencia) {
      this.salvarRecorrencia(id, obrigacao.recorrencia);
    }
  ...
  // Buscar por ID
  buscarPorId(id: string): Obrigacao | undefined {
    const stmt = db.prepare('SELECT * FROM obrigacoes WHERE id = ?');
    const obrigacao = stmt.get(id) as any;
  ...
  // Listar todas
  listarTodas(): Obrigacao[] {
    const stmt = db.prepare('SELECT * FROM obrigacoes ORDER BY dataVencimento ASC');
    const obrigacoes = stmt.all() as any[];

    return obrigacoes.map(o => this.mapearObrigacao(o));
  }

  // Filtrar obrigações
  filtrar(filtro: FiltroObrigacoes): Obrigacao[] {
    ...
    const obrigacoes = stmt.all(...params) as any[];

    return obrigacoes.map(o => this.mapearObrigacao(o));
  }

  // Atualizar
  atualizar(id: string, dados: Partial<Obrigacao>): Obrigacao | undefined {
    const campos: string[] = [];
    const valores: any[] = [];
    ...
    if (dados.recorrencia) {
      this.atualizarRecorrencia(id, dados.recorrencia);
    }
  ...
  deletar(id: string): boolean {
    const stmt = db.prepare('DELETE FROM obrigacoes WHERE id = ?');
    const result = stmt.run(id);
    return result.changes > 0;
  }

  // Salvar recorrência
  private salvarRecorrencia(obrigacaoId: string, recorrencia: Recorrencia) {
    const stmt = db.prepare(`
  ...
  private atualizarRecorrencia(obrigacaoId: string, recorrencia: Recorrencia) {
    const stmt = db.prepare(`
  ...
  // Buscar recorrência
  private buscarRecorrencia(obrigacaoId: string): Recorrencia | undefined {
    const stmt = db.prepare('SELECT * FROM recorrencias WHERE obrigacaoId = ?');
    const rec = stmt.get(obrigacaoId) as any;
  ...
  private mapearObrigacao(row: any): Obrigacao {
    return {
      id: row.id,
      titulo: row.titulo,
      ...
      recorrencia: this.buscarRecorrencia(row.id),
  ...
  // Salvar histórico
  salvarHistorico(historico: Omit<HistoricoAlteracao, 'id' | 'timestamp'>): void {
    const id = uuidv4();
  ...
  // Buscar histórico
  buscarHistorico(obrigacaoId: string): HistoricoAlteracao[] {
    const stmt = db.prepare(`
    ...
    const registros = stmt.all(obrigacaoId) as any[];
