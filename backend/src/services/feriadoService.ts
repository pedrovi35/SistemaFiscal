...
import { parseISO, isWeekend, addDays, format, isSameDay } from 'date-fns';
...
export class FeriadoService {
  ...
  async buscarFeriadosNacionais(ano: number): Promise<Feriado[]> {
    ...
    try {
      ...
      this.salvarFeriadosNoBanco(feriados);
  ...
  private salvarFeriadosNoBanco(feriados: Feriado[]) {
    const stmt = db.prepare(`
      INSERT OR REPLACE INTO feriados (data, nome, tipo)
      VALUES (?, ?, ?)
    `);

    const inserirMultiplos = db.transaction((feriadosList: Feriado[]) => {
      for (const feriado of feriadosList) {
        stmt.run(feriado.data, feriado.nome, feriado.tipo);
      }
    });

    inserirMultiplos(feriados);
  }

  // Buscar feriados do banco (fallback)
  private buscarFeriadosDoBanco(ano: number): Feriado[] {
    const stmt = db.prepare(`
      SELECT * FROM feriados 
      WHERE data LIKE ?
    `);

    return stmt.all(`${ano}-%`) as Feriado[];
  }
