...
const ParcelamentoModal: React.FC<ParcelamentoModalProps> = ({ parcelamento, onSave, onClose, clientes = [] }) => {
	...
	const [mostrarRecorrencia, setMostrarRecorrencia] = useState(!!parcelamento?.recorrencia);
					...
					<div className="border-t border-gray-200 dark:border-gray-700 pt-6">
						<div className="flex items-center mb-4">
							...
							<label className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">
								🔄 Configurar Recorrência Automática das Parcelas
							</label>
						</div>
						...
						{mostrarRecorrencia && (
							<div className="space-y-6 pl-6 bg-blue-50 dark:bg-gray-900/50 rounded-lg p-4">
								...
								<div>
									...
									/>
									<p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
										💡 O sistema criará a próxima parcela neste dia do mês. O vencimento será sempre no dia <strong>{formData.dataVencimento ? new Date(formData.dataVencimento + 'T00:00:00').getDate() : '___'}</strong> (baseado na Data de Vencimento acima)
									</p>
								</div>
